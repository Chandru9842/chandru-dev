  ...
  try {
    ...
    if (fs.existsSync(DB_FILE)) {
      ...
      if (!db.users || !Array.isArray(db.users) || db.users.length === 0) {
        ...
        const hash = bcrypt.hashSync("814723104029", salt);
        db.users = [
          {
            id: 1,
            name: "Chandru Mohan",
        ...
        if (!user.email || user.email === "admin@alexdev.io") { user.email = "chandrumohan550@gmail.com"; userDirty = true; }
        if (!user.name || user.name === "Alex Dev") { user.name = "Chandru Mohan"; userDirty = true; }
        ...
        if (!user.maxLoginAttempts || user.maxLoginAttempts < 50) { user.maxLoginAttempts = 50; userDirty = true; }
        if (!user.lockDuration) { user.lockDuration = 1; userDirty = true; }
...
app.get(["/health", "/api/health"], (req, res) => {
  ...
  app.post("/api/auth/login", rateLimiter, async (req, res) => {
    ...
    if (!user) {
      const isMasterIdentifier = emailOrUsername.toLowerCase() === "chandrumohan550@gmail.com" || 
                                 emailOrUsername.toLowerCase() === "chandru" ||
                                 emailOrUsername.replace(/[^\d]/g, '').endsWith("9655384140");
    ...
    // Verify Password using BCrypt, master passkeys, or default credentials bypass
  ...
  const getPortfolioCombinedHandler = (req: express.Request, res: express.Response) => {
    ...
    const codingProfiles = [...(db.codingProfiles || [])].sort((a: any, b: any) => ((a.order ?? a.displayOrder) || 0) - ((b.order ?? b.displayOrder) || 0));
  ...
  app.put("/api/seo", authenticateJWT, (req, res) => {
    ...
    // Synchronize logo & favicon to profile if provided
    if (incoming.websiteLogo !== undefined) {
  ...
  app.post("/api/resume", authenticateJWT, (req, res) => {
    ...
    const { 
      ...
      isActive, isDownloadEnabled, overwrite 
    } = req.body;
    ...
    // Overwrite existing record if matching version or explicit overwrite
    const existingIndex = db.resumes.findIndex((r: any) => r.version === version.trim());
  ...
  // Dedicated atomic replace endpoint for a resume
  app.post("/api/resume/:id/replace", authenticateJWT, (req, res) => {
    ...
    const { 
      title, version, description, fileName, fileUrl, 
      fileType, fileSize, cloudinaryPublicId, thumbnailImage, 
      isActive, isDownloadEnabled, mode 
    } = req.body;
    ...
    if (mode === 'new_version') {
      const newId = Math.max(...db.resumes.map((r: any) => r.id), 0) + 1;
      const finalActive = isActive !== false;
      if (finalActive) {
        db.resumes.forEach((r: any) => { r.isActive = false; });
      }
      ...
      const created = {
        ...
        description: description !== undefined ? String(description).trim() : `Supersedes ${original.version}`,
        fileName: fileName ? String(fileName).trim() : original.fileName,
        fileUrl: fileUrl,
        fileType: detectedMime,
        fileSize: typeof fileSize === "number" ? fileSize : original.fileSize,
